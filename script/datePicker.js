...
Calendar_datePicker.prototype = {
    ...
    bindEvt_chooseDate: function() {
        ...
        $calendar_body.delegate('td[class!="disabled"][class!="space"]', 'click', function(e) {
            ...
            if(_this.ops.byWeek){
                if(!_this.begin){
                    date=$(this).parents('tr').find('td:first').attr('date');
                }else{
                    date=$(this).parents('tr').find('td:last').attr('date');
                }
            }
            if (_this.ops.double) {
                _this.setTempDate(date);
            } else {
                _this.setDate(date);
                if (typeof _this.ops.sure == 'function') {
                    _this.ops.sure(date);
        ...
        if (_this.partner != null) {
            $calendar_td.hover(function() {
                ...
                if(_this.ops.byWeek){
                    if(_this.begin){
                        thisDate=$(this).parents('tr').find('td:last').attr('date');
                    }else{
                        thisDate=$(this).parents('tr').find('td:first').attr('date');
                    }
                }else{
    ...
    calendarBox: function() {
        ...
        if (this.partner != null && !this.begin) {
            ...
            if(this.ops.byWeek){
                boxClass+=' calendar_box_end_week'
            }
        } else {
            boxClass = 'calendar_box_begin';
            if(this.ops.byWeek){
                boxClass+=' calendar_box_begin_week'
            }
        }
    ...
    calendarGrid: function(day) {
        ...
        days=this.getMonthDaysNum(month,year);
        /*how many days in preMonth*/
        preMonthDays=this.getMonthDaysNum(preMonth,preYear);
        /*how many days in nextMonth*/
        nextMonthDays=this.getMonthDaysNum(nextMonth,nextYear);
        /*draw*/
        var a = [];
        if(this.ops.hasBefore){
            for(var j = preMonthDays-firstDay+1; j<=preMonthDays;j++){
                a.push('<td date="'+ preYear + '/' + preMonth + '/' + j +'"><a>'+j+'</a></td>');
            }
        }else{
        ...
        if (l % 7 != 0) {
            for (var k = 0; k < 7 - (l % 7); k++) {
                if(this.ops.hasAfter){
                    a.push('<td date="'+ nextYear + '/' + nextMonth + '/' + (k+1) +'"><a>' + (k+1) + '</a></td>');
                }else{
                    a.push('<td class="space"></td>');
                }
            }
                ...
                if (begin) {
                    ...
                    if (this.largerThanByYM(partnerDateYM, thisView) && this.largerThanByYM(thisView, thisDateYM)) {
                        ...
                        if (thisView == thisDateYM || (this.ops.byWeek && closeMonth)) {
                            ...
                            if(this.ops.byWeek){
                                firstTD = $calendar.find('td[date="' + thisDate + '"]').parents('tr').find('td:last');
                            }
                        ...
                        if (thisView == partnerDateYM) {
                            ...
                            if(this.ops.byWeek){
                                lastTD=$calendar.find('td[date="' + partnerDate + '"]').parents('tr').find('td:first');
                            }
                    ...
                    if (this.largerThanByYM(partnerDate, thisView)) {
                        ...
                        if (thisView == thisDateYM || (this.ops.byWeek && closeMonth)) {
                            ...
                            if(this.ops.byWeek){
                                lastTD=$calendar.find('td[date="' + thisDate + '"]').parents('tr').find('td:first');
                            }
                            if(!this.ops.byWeek){
                                lastTD.prevAll().removeClass('disabled blueDate yellowDate').addClass(color);
                            }else{
                                lastTD.prevUntil('tr:first').removeClass('disabled blueDate yellowDate').addClass(color);
                            }
                            lastTD.parent().prevAll().find('td').removeClass('disabled blueDate yellowDate').addClass(color);
                        }
                    }
    ...
    getMonthDaysNum: function(month,year) {
        var days=0;
        if (month == 2) {
            if (this.isLeap(year)) {
                days = 29;
    ...
    syncInput: function() {
        var date = this.getDate();
        if(this.ops.byWeek){
            var weekBegin=new Date(+new Date(date)-86400000*6);
            this.input.val(weekBegin.getFullYear()+'/'+(weekBegin.getMonth()+1)+'/'+weekBegin.getDate()+'-'+date);
        }else{
            this.input.val(date);
        }
    },
...
$.fn.extend({
    datePicker: function(ops) {
                ...
                shell.delegate('.sure', 'click', function(e) {
                    ...
                    if (beginDate != '' && endDate != '') {
                        ...
                        if (verify) {
                            ...
                            if($input.ops.byWeek){
                                beginDate=new Date(+new Date(beginDate)-86400000*6);
                                beginDate=beginDate.getFullYear()+'/'+(beginDate.getMonth()+1)+'/'+beginDate.getDate();

                                endDate=new Date(+new Date(endDate)+86400000*6);
        ...
        var defaults = {
            ...
            hasBefore: false, //是否渲染1号前面的日期
            hasAfter:false,//是否渲染最后一天后面的日期
            byWeek:false,
        ...
        if(ops.beginDate!=='' && ops.double && ops.byWeek){
            var beginDate=new Date(+new Date(ops.beginDate)+86400000*6);
            beginDate=beginDate.getFullYear()+'/'+(beginDate.getMonth()+1)+'/'+beginDate.getDate();
            ops.beginDate=beginDate;
        }
        if(ops.endDate!=='' && ops.double && ops.byWeek){
            var endDate=new Date(+new Date(ops.endDate)-86400000*6);
            endDate=endDate.getFullYear()+'/'+(endDate.getMonth()+1)+'/'+endDate.getDate();
            ops.endDate=endDate;
        }
